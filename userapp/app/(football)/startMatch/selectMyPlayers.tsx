// app/(football)/startMatch/selectMyTeamPlayers.tsx
...
export default function SelectMyTeamPlayersScreen() {
  ...
  useEffect(() => {
    if (matchData.myTeam.selectedPlayers.length > 0) {
      setSelectedPlayers(matchData.myTeam.selectedPlayers);
    }
  ...
  const handleProceed = useCallback(async () => {
    ...
    try {
      ...
      updateMyTeamPlayers(selectedPlayers);
      
      // Navigate to opponent team player selection
      router.push('/(football)/startMatch/selectOpponentPlayers');
  ...
  }, [selectedPlayers, matchData.playersPerTeam, updateMyTeamPlayers, router, setError]);
            ...
            <View className="flex-row items-center mb-2">
              ...
              {player.experience && (
                <View className={`ml-2 px-2 py-1 rounded-lg ${
                  isSelected ? 'bg-green-200' : 'bg-slate-100'
                }`}>
