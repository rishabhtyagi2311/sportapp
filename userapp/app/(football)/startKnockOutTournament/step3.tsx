...
export default function KnockoutStep3() {
  ...
  const totalMatches = useMemo(() => (draft ? draft.teamIds.length - 1 : 0), [draft]);
  const totalRounds = useMemo(() => (draft ? Math.log2(draft.teamIds.length) : 0), [draft]);
  ...
  const handleCreate = async () => {
    ...
    updateDraft({
      ...
      duration,
      extraTimeAllowed: allowExtraTime,
    ...
    try {
      ...
      router.replace({
        pathname: '/(football)/startKnockOutTournament/[tournamentId]/dashboard',
        ...
        <View className="bg-slate-100 rounded-xl p-4 mb-6">
          <View className="flex-row items-start">
            ...
            <Text className="flex-1 text-xs text-slate-600 ml-2 leading-5">
              Referees are entered per match once the tournament starts. A tied knockout match requires a penalty shootout to determine the winner.
            </Text>
          </View>
        </View>
