...
function mapMatchForClient(match) {
    return {
        ...match,
...
function mapMatchEventForClient(event) {
    return {
        ...event,
...
function computePlayerStats(homeRoster, awayRoster, homeTeamId, awayTeamId, events, duration) {
    const stats = new Map();
    const initPlayer = (playerId, teamId, isStarter) => {
    ...
    for (const event of events) {
        if (event.eventType === "goal" && event.playerId) {
            ...
            if (event.relatedPlayerId) {
                ...
                if (goingOff)
                    goingOff.minutesPlayed = event.minute;
            }
        }
    }
...
async function applyFixtureResult(tx, fixture, match) {
...
class MatchService {
    static async createMatch(userId, data) {
        const [homeTeam, awayTeam] = await Promise.all([
            index_1.prisma.footballTeam.findUnique({ where: { id: data.homeTeamId } }),
            index_1.prisma.footballTeam.findUnique({ where: { id: data.awayTeamId } }),
        ]);
    ...
    static async startMatch(userId, matchId) {
        const match = await index_1.prisma.match.findFirst({ where: { id: matchId, creatorId: userId } });
        if (!match) {
        ...
        const updated = await index_1.prisma.match.update({
            where: { id: matchId },
    ...
    static async abandonMatch(userId, matchId, reason) {
        const match = await index_1.prisma.match.findFirst({ where: { id: matchId, creatorId: userId } });
        if (!match) {
            throw new Error("Match not found or not owned by you");
        }
    ...
    static async addEvent(userId, matchId, data) {
        const match = await index_1.prisma.match.findFirst({ where: { id: matchId, creatorId: userId } });
        if (!match) {
            throw new Error("Match not found or not owned by you");
        }
    ...
    static async updatePossession(userId, matchId, data) {
        const match = await index_1.prisma.match.findFirst({ where: { id: matchId, creatorId: userId } });
        if (!match) {
            throw new Error("Match not found or not owned by you");
        }
    ...
    static async endMatch(userId, matchId, data) {
        const match = await index_1.prisma.match.findFirst({ where: { id: matchId, creatorId: userId } });
        if (!match) {
            throw new Error("Match not found or not owned by you");
        }
        ...
        const fixture = await index_1.prisma.tournamentFixture.findUnique({
            where: { matchId },
        ...
        return index_1.prisma.$transaction(async (tx) => {
            ...
            const homeRoster = match.homeRoster;
            const awayRoster = match.awayRoster;
    ...
    static async getTeamMatches(teamId) {
        const matches = await index_1.prisma.match.findMany({
    ...
    static async getPlayerStats(profileId) {
        const aggregate = await index_1.prisma.matchPlayerStat.aggregate({
