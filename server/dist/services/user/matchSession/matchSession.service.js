...
exports.UserMatchSessionService = void 0;
...
class UserMatchSessionService {
    static async listAvailable(filters = {}) {
        const where = {
        ...
        const sessions = await index_1.prisma.matchSession.findMany({
            where,
    ...
    static async getById(sessionId) {
        const session = await index_1.prisma.matchSession.findUnique({
            where: { id: sessionId },
            include: { venue: { select: { name: true } } },
        });
        if (!session) {
    ...
    static async getMySessions(userId) {
        const participations = await index_1.prisma.matchSessionParticipant.findMany({
    ...
    static async leaveSession(userId, sessionId) {
        return index_1.prisma.$transaction(async (tx) => {
            const session = await tx.matchSession.findUnique({ where: { id: sessionId } });
            if (!session) {
                throw new Error("Match session not found");
...
exports.UserMatchSessionService = UserMatchSessionService;
//# sourceMappingURL=matchSession.service.js.map