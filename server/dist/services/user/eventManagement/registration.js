...
function mapRegistrationForClient(registration) {
    ...
    return {
        ...
        ...(event ? { event: (0, event_1.mapEventForClient)(event) } : {}),
    };
}
class EventRegistrationService {
    static async createRegistration(userId, eventId, data) {
        ...
        try {
            if (data.participationType === "individual") {
                const registration = await index_1.prisma.eventRegistration.create({
                    data: { eventId, userId, status: "pending" },
                    include: { user: true },
                });
                return mapRegistrationForClient(registration);
            }
            const footballProfile = await index_1.prisma.footballProfile.findUnique({ where: { userId } });
            if (!footballProfile) {
                throw new Error("You need a football profile to register a team");
            }
            const team = await index_1.prisma.footballTeam.findUnique({ where: { id: data.footballTeamId } });
            if (!team) {
    ...
    static async getMyRegistrations(userId) {
        const footballProfile = await index_1.prisma.footballProfile.findUnique({ where: { userId } });
        const registrations = await index_1.prisma.eventRegistration.findMany({
            where: {
                OR: [
                    { userId },
    ...
    static async getRegistrationsForEvent(userId, eventId) {
        const event = await index_1.prisma.event.findFirst({ where: { id: eventId, creatorId: userId } });
        if (!event) {
    ...
    static async processRegistration(userId, eventId, registrationId, data) {
        const event = await index_1.prisma.event.findFirst({ where: { id: eventId, creatorId: userId } });
        if (!event) {
            throw new Error("Event not found or not owned by you");
        }
