...
export default function MatchSessionVenueSelector() {
      ...
      <FlatList
        ...
        renderItem={({ item, index }) => (
          <FadeInView delay={Math.min(index, 6) * 60}>
            <AnimatedPressable
              onPress={() => router.push({
                pathname: "/(venueManagement)/slotHandling/matchSessionDashboard",
            ...
            >
              <View className="w-14 h-14 bg-blue-50 rounded-[20px] items-center justify-center mr-4">
                <FontAwesome5 name="users" size={22} color="#2563eb" />
              </View>
