...
const MetricCard = ({
  ...
  <FadeInView delay={delay} className="flex-1">
    <View className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm items-start justify-between min-h-[110px]">
...
export default function VenueOwnerDashboard() {
  ...
  const { venues, fetchMyVenues } = useVenueStore();

  // Refetch venues every time the dashboard comes into focus
  useFocusEffect(
    useCallback(() => {
      fetchMyVenues();
    }, [fetchMyVenues])
  ...
  const totalSlots = venues.reduce(
    (sum, v) => sum + (v.timeSlotCount ?? v.timeSlots.length),
    0
  );
